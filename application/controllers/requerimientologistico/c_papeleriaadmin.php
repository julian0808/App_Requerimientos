...
//            }
     ...
     public function fc_filtrotabla()
  {
         ...
         $datos = '';
            ...
            $observacionadmin ='<div class=" col-md-12" ><textarea class="form-control" '.$inhabilitarespuesta.' '.$inhabilitarespuesta2.' id="observacionad'.$valor->idpapeleria.'"  name="observacionad'.$valor->idpapeleria.'" rows="2">'.$valor->observacionadminp.'</textarea> </div>' ;
 
            $respuesta = '<select class="form-control input-sm  " '.$inhabilitarespuesta.' '.$inhabilitarespuesta2.' id="respuestaadmin'.$valor->idpapeleria.'" name="respuestaadmin'.$valor->idpapeleria.'"  >';
...
//style="display:none"
            ...
            $datos .=  '<tr '.$colorfila.'>'
                   ...
                   .'<td>'.$valor->estadorequerimiento.'</td>'
                        ...
                        .'<td>'.$valor->observacion_papeleria.'</td>'
                        .'<td>'.$valor->fecharegistro.'</td>'
                        .'<td><a class="btn btn-success btn-sm fancybox" id="editar" '.$esconde.' '.$cancelado.' href="'.DESCARGAR.'/'.$valor->archivo_papeleria.'" role="button"><span class="glyphicon glyphicon-download"></span> Descargar</button></td>'
                        .'<td>'.$respuesta.'</td>'
                        .'<td>'.$observacionadmin.'</td>'
                        //.'<td><a class="btn btn-success btn-sm fancybox" '.$archivo.' id="editar" href="'.DESCARGAR.'/'.$valor->nombre_archivo.'" role="button"><span class="glyphicon glyphicon-download"></span> Descargar</a></td>'
                        .'<td><button type="button" id="guardarpapeleriaadmin"  '.$cancelado.''.$inhabilitarespuesta.' onclick="fc_guardarpapeleriaadmin('.$comilla.$valor->idpapeleria.$comilla.')" class="btn btn-success btn-sm"><span class="glyphicon glyphicon-edit"></span> Guardar </button></td>';
