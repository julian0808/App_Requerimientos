...
<!--        <input type="text" class="form-control input-sm" id="idpapeleriaadmin" name="idpapeleriaadmin" style="display: none"     placeholder="rol" value="<?php echo($idpapeleriaadmin); ?>">-->
...
<!--                                        <table class="table table-bordered table-hover table-striped">
                                            <thead>
                                                <tr>
                                                    ...
                                                    <th>Estado req</th>
                                                    <th>Nombre</th>
                                                    <th>Fecha Papeleria</th>
                                                    <th>Hora de Papeleria</th>
                                                    <th>Cantidad</th>
                                                    <th>Observacion</th>
                      ...
                      <table class="table table-bordered" id="tr24h2" style="table-layout: fixed">
                          <thead>
                              <tr class="success">
                                  ...
                                  <th style="width:100px;">Hora de Papeleria</th>
                                  <th style="width:100px;">Solicita toner</th>
                                  <th style="width:200px;">Cantidad</th>
                                  <th style="width:300px;">Observacion</th>
                                  <th style="width:300px;">Fecha Registro</th>
...
{ //alert(div + ' juanpis ' + campo);
   ...
   { 
    ...
    validarguardarpapeleriaadmin();
}


function validarguardarpapeleriaadmin()
{
    if ($('#fechast').val() != '' && $('#horast').val() != '' && $('#lugarpst').val() != ''&& $('#lugarlst').val() != '' && $('#cantidadpt').val() != '' && $('#estadot').val() != '' )
        
    {
       $('#guardarpapeleriaadmin').prop('disabled', false);
           
   }
   else
   {
       $('#guardarpapeleriaadmin').prop('disabled', true);
...
function fc_guardarpapeleriaadmin(idpapeleria)
{
     administrativomail(idpapeleria, 'Transporte');
    
                        $.ajax({
                          url: "fc_guardarpapeleriaadmin",
                          type: "GET",
                          data: {vidpapeleria:  idpapeleria,
                                 vrespuestaadmin : $('#respuestaadmin' + idpapeleria).val(),
                                 vobservacionad : $('#observacionad' + idpapeleria).val()
                                 },    
                          dataType: "html",
                          success : function(obj){   
                              //location.href = "../c_papeleriaadmin/fc_papeleriaadmin?idpapeleriaadmin="+ obj;
                          filtrartabla();
                          botonguardar();
                          }
                        });                    
    ...
    //NO TOCAR - voler a capitulos                
        ...
        $("#actualizar").click(function ()
        {
                        $.ajax({
                          url: "fc_actualizar",
                          type: "GET",
                          data: {vfolio : $('#folio').val(), vvalorpapeleriaadmin : $('#valorpapeleriaadmin').val()},    
                          dataType: "html",
                          success : function(obj){                      
                          botonactualizar();
                          }
