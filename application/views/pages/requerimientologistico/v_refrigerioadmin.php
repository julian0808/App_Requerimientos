...
<!--        <input type="text" class="form-control input-sm" id="apellido" name="apellido"  placeholder="apellido" value="<?php echo($carraydatosgenerales['eapellido1']); ?>">-->
<!--        <input type="text" class="form-control input-sm" id="idrefrigerioadmin" name="idrefrigerioadmin" style="display: none"     placeholder="rol" value="<?php echo($idrefrigerioadmin); ?>">-->
                      ...
                      <table class="table table-bordered" id="tr24h2" style="table-layout: fixed">
                          <thead>
                              <tr class="success">
                                  ...
                                  <th style="width:100px;">Estado req </th>
                                  ...
                                  <th style="width:300px;">Descargar</th>
                                  <th style="width:300px;">Observacion</th>
                                  <th style="width:130px;">Fecha registro</th>
                                  <th style="width:150px;">Respuesta admin</th>
                                  <th style="width:200px;">Observacion Admin</th>
...
{ //alert(div + ' juanpis ' + campo);
   ...
   { 
    ...
    validarguardarrefrigerioadmin();
}


function validarguardarrefrigerioadmin()
{
    if ($('#fechast').val() != '' && $('#horast').val() != '' && $('#lugarpst').val() != ''&& $('#lugarlst').val() != '' && $('#cantidadpt').val() != '' && $('#estadot').val() != '' )
        
    {
       $('#guardarrefrigerioadmin').prop('disabled', false);
           
   }
   else
   {
       $('#guardarrefrigerioadmin').prop('disabled', true);
...
function fc_guardarrefrigerioadmin(idrefrigerio)
{
    administrativomail(idrefrigerio, 'Refrigerios');
    
                        $.ajax({
                          url: "fc_guardarrefrigerioadmin",
                          type: "GET",
                          data: {vidrefrigerio:  idrefrigerio,
                                 vrespuestaadmin : $('#respuestaadmin' + idrefrigerio).val(),
                                 vobservacionad : $('#observacionad' + idrefrigerio).val()
                                 },    
                          dataType: "html",
                          success : function(obj){   
                              //location.href = "../c_refrigerioadmin/fc_refrigerioadmin?idrefrigerioadmin="+ obj;
        filtrartabla();                 
        botonguardar();
                          }
                        });                    
    ...
    //NO TOCAR - voler a capitulos                
        ...
        $("#actualizar").click(function ()
        {
                        $.ajax({
                          url: "fc_actualizar",
                          type: "GET",
                          data: {vfolio : $('#folio').val(), vvalorrefrigerioadmin : $('#valorrefrigerioadmin').val()},    
                          dataType: "html",
                          success : function(obj){                      
                          botonactualizar();
                          }
