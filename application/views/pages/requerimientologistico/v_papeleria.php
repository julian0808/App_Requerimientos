...
<link rel="icon" href="<?php echo IMAGEN,'corazon.ico';?> " type="image/x-icon">
...
<!--        <input type="text" class="form-control input-sm" id="rol" name="rol"      placeholder="rol" value="<?php echo($crol); ?>">-->
<!--        <input type="text" class="form-control input-sm" id="idpapeleria" name="idpapeleria" style="display: none"     placeholder="rol" value="<?php echo($idpapeleria); ?>">-->
...
<div class="row">
    <div class="col-md-2 ">
        <label>id papeleria</label><br>
        <input type="text" class="form-control input-sm" id="idpapeleria" name="idpapeleria" style="display:none"  placeholder="documento" value="<?php echo($idpapeleria); ?>"><?php echo($idpapeleria); ?>
    </div>
     <div class="col-md-2">
...
<div class="row ">
     ...
     <div id="fechaspdiv" <?php if($carraydatospapeleria['fechap']==''){echo('class="col-md-2 col-md-offset-2 has-error"');}else{echo('class="col-md-2 col-md-offset-2"');} ?>>
         <label class="control-label">Fecha del servicio</label><br>
         <input type="date" class="form-control input-sm " id="fechasp" name="fechasp" onchange="validarbolitasytextoformulario('fechaspdiv','fechasp')"   placeholder="fechasp" onkeyup="eliminardigito()" value="<?php  echo($carraydatospapeleria['fechap']); ?>">
    </div>
    
    <div id="horaspdiv" <?php if($carraydatospapeleria['horap']==''){echo('class="col-md-2 has-error"');}else{echo('class="col-md-2"');} ?>>
        <label class="control-label">Hora del servicio</label><br>
        <input type="time" class="form-control input-sm" id="horasp" name="horasp" onchange="validarbolitasytextoformulario('horaspdiv','horasp')"   placeholder="horasp" value="<?php  echo($carraydatospapeleria['horap']); ?>">
    </div>
    
    
    <div id="tonerdiv"  <?php if($carraydatospapeleria['toner']==''){echo'class="col-sm-2 has-error"';}else{echo'class="col-sm-2"';}  ?>>
                  <label class="control-label" > Desea solicitar toner?</label>
                  <select class="form-control input-sm  " id="toner"   name="toner"  onchange="validarbolitasytextoformulario('tonerdiv','toner')" >   
                      
                        <?php  if($carraydatospapeleria['toner']=='')
                      {
                      ...
                      {
                          echo '<option value="'.$carraydatospapeleria['toner'].'">'.$carraydatospapeleria['toner'].'</option>'
                                  . ' 
                      <option value="SI">SI</option>
                      <option value="NO">NO</option>';
    ...
    <div id="cantidadpdiv" <?php if($carraydatospapeleria['cantidadp']==''){echo('class="col-md-2 has-error"');}else{echo('class="col-md-2"');} ?>>
        <label class="control-label">Cantidad</label><br>
        <input type="text" class="form-control input-sm" id="cantidadp" onkeypress="return soloNumeros(event)" name="cantidadp" onchange="validarbolitasytextoformulario('cantidadpdiv','cantidadp')"  placeholder="Digite lugar de salida" value="<?php  echo($carraydatospapeleria['cantidadp']); ?>">
    </div><hr><br><br>
...
<div id="observacionpdiv" class="row  " >
      
          <label class="col-md-offset-5 control-label">Observacion</label><br>
          <textarea class="form-control" id="observacionp" onchange="validarbolitasytextoformulario('observacionpdiv','observacionp')" name="observacionp" rows="4"><?php  echo($carraydatospapeleria['observacionp']); ?></textarea>              
...
<div class="row">
    <div class="col-md-2 col-md-offset-5">
       <button type="button" id="guardarpapeleria" disabled="disabled"  name="guardarpapeleria" class="btn btn-primary">Guardar</button>
<!-- <p>Se�or usuario en el momento la pagina no se encuentra disponible para crear un nuevo requerimiento logistico, por favor evaluar los pendientes</p>-->
    </div>
</div>
...
{ //alert(div + ' juanpis ' + campo);
   ...
   { 
    ...
    validarguardarpapeleria();
}


function validarguardarpapeleria()
{
    if ($('#fechasp').val() != '' && $('#horasp').val() != ''  && $('#toner').val() != '' && $('#lugarpsr').val() != ''&& $('#lugarlsr').val() != '' && $('#cantidadp').val() != '' && $('#tipopapeleria').val() != '' )
        
    {
       $('#guardarpapeleria').prop('disabled', false);
           
   }
   else
   {
       $('#guardarpapeleria').prop('disabled', true);
    ...
    //NO TOCAR - voler a capitulos                
        ...
        $("#guardarpapeleria").click(function ()
        {
                        $.ajax({
                          url: "fc_guardarpapeleria",
                          type: "GET",
                          data: {vdocumento : $('#documento').val(),
                                 vfechasp : $('#fechasp').val(),
                                 vhorasp : $('#horasp').val(),
                                 vtoner : $('#toner').val(),
                                 vcantidadp : $('#cantidadp').val(),
                                 vnameFile : $('#nameFile').val(),
                                 vobservacionp : $('#observacionp').val()},    
                          dataType: "html",
                          success : function(obj){   
                              location.href = "../c_papeleria/fc_papeleria?idpapeleria="+ obj;
                          botonguardar();
        ...
        $("#actualizar").click(function ()
        {
                        $.ajax({
                          url: "fc_actualizar",
                          type: "GET",
                          data: {vfolio : $('#folio').val(), vvalorpapeleria : $('#valorpapeleria').val()},    
                          dataType: "html",
                          success : function(obj){                      
                          botonactualizar();
                          }
