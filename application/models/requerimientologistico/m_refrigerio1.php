...
class M_refrigerio extends CI_Model {
 ...
 public function fm_cargardatosgenerales($mdocumento)
    ...
    public function fm_guardarrefrigerio($mvdocumento,$mvfechasr,$mvhorasr,$mvlugarlsr,$mvcantidadpr,$mvobservacionr)
    {
        $query =$this->db->query('call spguardarrefrigerio("'.$mvdocumento.'","'.$mvfechasr.'","'.$mvhorasr.'","'.$mvlugarlsr.'",'.$mvcantidadpr.',"'.$mvobservacionr.'");');
        $resultado = $query->result();
        $query->next_result(); //NO SE TOCAN Estas dos funciones permiten realizar varias consultas en el mismo controlador.
        $query->free_result(); //NO SE TOCAN  Estas dos funciones permiten realizar varias consultas en el mismo controlador.
       return $resultado;
