...
class M_transporte extends CI_Model {
 ...
 public function fm_cargardatosgenerales($mdocumento)
    ...
    public function fm_guardartransporte($mvdocumento,$mvfechast,$mvhorast,$mvlugarpst,$mvlugarlst,$mvcantidadpt,$mvobservaciont)
    {
        $query =$this->db->query('call spguardartransporte('.$mvdocumento.',"'.$mvfechast.'","'.$mvhorast.'","'.$mvlugarpst.'","'.$mvlugarlst.'",'.$mvcantidadpt.',"'.$mvobservaciont.'");');
        $resultado = $query->result();
        $query->next_result(); //NO SE TOCAN Estas dos funciones permiten realizar varias consultas en el mismo controlador."
        $query->free_result(); //NO SE TOCAN  Estas dos funciones permiten realizar varias consultas en el mismo controlador.
       return $resultado;
